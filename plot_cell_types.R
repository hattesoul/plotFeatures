...
# each line contains one cell type consisting of cell type name, active features (++), and inactive features (--)
# name and features must be comma-separted
# e. g. basal cells, ++TP63, ++KRT5, --MKI67, --SCGB3A1, --SCGB1A1, --MUC5B, --ELF3, --FOXJ1
celltype_file = "./assets/celltypes.txt"
...
if(!!file.access(celltype_file, mode = 4)){
  fileError(celltype_file)
}
cat(" done.\n")
...
# load file with cell types of interest
cat("loading cell types of interest from ", celltype_file, " ...", sep = "")
celltypes <- read.table(celltype_file, sep = ",", header = FALSE)
cat(" done.\n")
cat("process cell type data ...")

#celltypes <- read.table(celltype_file, sep = ",", header = FALSE)
cat(" done.\n")


cat("summary:\n  barcodes: ", attr(mat,'Dim')[2], "\n  features: ", attr(mat,'Dim')[1], "\n  cell types of interest: ", dim(celltypes)[1], "\n", sep = "")
